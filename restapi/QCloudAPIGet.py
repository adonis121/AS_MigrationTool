...
def Invoke_QCloudAPIGet(API: str, Raw: bool = False, Session: Optional[Session] = None, Timeout: int = 86400):
    """
    Invoke a QCloud API GET request.
    ...
    response = requests.get(paramInvokeWebRequest['url'], headers=paramInvokeWebRequest['headers'], timeout=Timeout)
...
response = Invoke_QCloudAPIGet(API, Raw, session, Timeout)
print(response)
